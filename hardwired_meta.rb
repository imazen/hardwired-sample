module Hw
    ...
    def pages
      in_category = Hw::ContentFile.find_all.select do |page|
    ...
    def articles
      Hw::ContentFile.find_articles.select { |article| article.categories.include?(self) }
    end
    ...
    def inline_summary
     ...
     def articles_by_tags
       Hw::ContentFile.find_articles.select { |article| not (article.tags & self.tags).empty? }
     end
     def self.articles_by_tag(tag)
        Hw::ContentFile.find_articles.select { |article| not (article.tags & [tag]).empty? }
      end
